document.addEventListener('DOMContentLoaded', () => {
    ...
    btnLlamar.onclick = async () => {
        try {
            ...
            if(res.ok) {
                ...
                if(turno.id_cliente) await cargarClienteId(turno.id_cliente);
    ...
    function renderProductos(data) {
        ...
        data.creditos.forEach(c => {
            cr.innerHTML += `<div class="card producto-card"><h4>${c.tipo_credito}</h4><p>Deuda: $${c.saldo_pendiente.toLocaleString()}</p><div class="card-acciones"><button class="btn btn-verde btn-small" onclick="abrirModalPagarCredito(${c.id})">Abonar</button><button class="btn btn-secondary btn-small" onclick="verAmortizacion(${c.id})">Amortización</button></div></div>`;
        });
        ...
        data.tarjetas_credito.forEach(t => {
             let btnPagar = t.cupo_usado > 0 ? `<button class="btn btn-verde btn-small" onclick="abrirModalPagarTarjeta(${t.id})">Pagar</button>` : `<span style="color:green;font-weight:bold">Al día</span> <button class="btn btn-gris btn-small" onclick="eliminarTarjeta(${t.id})">Eliminar</button>`;
             tj.innerHTML += `<div class="card producto-card"><h4>Tarjeta **** ${t.numero_tarjeta.slice(-4)}</h4><p>Usado: $${t.cupo_usado.toLocaleString()} / <strong>$${t.cupo_total.toLocaleString()}</strong></p><div class="card-acciones">${btnPagar}<button class="btn btn-info btn-small" onclick="abrirModalAvance(${t.id}, ${t.cupo_total-t.cupo_usado})">Avance</button></div></div>`;
        });
    ...
    function mostrarOpcionesNoAfiliado(nombre) {
        ...
        document.getElementById('cliente-tipo').textContent = "Visitante";
    ...
    window.opCajero = async (tipo, id) => {
        ...
        if(!m) return alert("Monto requerido");
        ...
        const d = await res.json(); alert(d.mensaje||d.error); if(res.ok) await cargarClienteId(CLIENTE_ID);
    };
    ...
    document.getElementById('btn-afiliar-cliente').onclick = async () => {
        ...
        await cargarClienteId(CLIENTE_ID);
    };
    ...
    window.verMovimientos = async (id, num) => {
        ...
        lista.innerHTML = '';
        data.forEach(t => {
            const color = (t.tipo === 'Retiro' || t.tipo === 'Pago') ? 'red' : 'green';
            const signo = (color === 'red') ? '-' : '+';
            lista.innerHTML += `<li><b>${t.tipo}</b> <span style="color:${color}">${signo}$${t.monto.toLocaleString()}</span> <br><small>${t.fecha}</small></li>`;
        });
